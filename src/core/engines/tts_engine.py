...
_config = load_config()
_tts_config = _config.get("engines", {}).get("tts", {})
...
class TTSEngine:
    ...
    def __init__(self):
        """
        Initialise the Qwen3-TTS engine with structured config and device mapping.
        """

        self.compile_flag = _tts_config.get("compile", False)
        self.default_speaker = _tts_config.get("default_speaker", "Dylan")
        self.temperature = _tts_config.get("default_temperature", 0.3)
        self.bitrate = _tts_config.get("default_bitrate", "192k")
        
        _logger.info(f"TTSEngine configuration parsed: speaker={self.default_speaker}, compile={self.compile_flag}")
        ...
        if self.compile_flag:
            _logger.info("Compiling Qwen3-TTS model for faster inference (torch.compile)...")
            try:
                self.model = torch.compile(self.model)
                _logger.info("Model compilation successful.")
            except Exception as e_comp:
                _logger.error(f"torch.compile failed: {e_comp}. Continuing with uncompiled model.")
